...
const Register = () => {
  ...
  const validate = (values) => {
    ...
    else if (!/[0-9]/.test(values.password)) errors.password = "Password must contain a number";
    else if (!/[!@#$%^&*(),.?":{}|<>]/.test(values.password)) errors.password = "Password must contain a special character";
    return errors;
  };
      ...
      <AuthCard title="Create Account" message="Let's get you started">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
          ...
          <Input name="password" type="password" value={values.password}
            onChange={handleChange} error={errors.password} placeholder="Create a password" />
        ...
        <div className="flex justify-center">
          <GoogleLogin onSuccess={handleGoogleSuccess} onError={handleGoogleError}
            text="signup_with" shape="rectangular" size="large" width="100%" />
        </div>
